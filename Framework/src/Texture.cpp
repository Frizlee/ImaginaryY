...
void Texture::create(std::vector<Image*> &&imgs, Renderer &renderer)
{
	...
	// Erase not valid images
	for (auto it = begin(imgs); it != end(imgs);)
	{
		if (*it == nullptr || (*it)->getFormat() != imageFormat ||
			(*it)->getWidth() != width || (*it)->getHeight() != height)
			it = imgs.erase(it);
		else
			++it;
	}
...
Image::Format Texture::getFormat()
{
	return mFormat;
}
